...
def overseas_order_revise(
    ...
    if dvsn not in OVERSEAS_ORD_DVSNS:
        dvsn = OVERSEAS_R_ORD_DVSNS.get(dvsn, None)  # type: ignore
        if not dvsn:
            raise ValueError(
                f'주문 구분은 ({", ".join(i if i else "None" for i in OVERSEAS_R_ORD_DVSNS)}) 중 하나여야 합니다.')
