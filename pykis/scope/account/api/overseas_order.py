...
TR_ID_CODE_MAP: list[dict[str, tuple[str, str]]] = [
    # [실전투자]
    {
        'NASD': ('JTTT1002U', 'JTTT1006U'),  # 미국
        'NYSE': ('JTTT1002U', 'JTTT1006U'),  # 미국
        'AMEX': ('JTTT1002U', 'JTTT1006U'),  # 미국
    ...
    # [모의투자]
    {
        'NASD': ('VTTT1002U', 'VTTT1006U'),  # 미국
        'NYSE': ('VTTT1002U', 'VTTT1006U'),  # 미국
        'AMEX': ('VTTT1002U', 'VTTT1006U'),  # 미국
...
def overseas_order(
    ...
    supported_dvsn = OVERSEAS_ORD_DVSN_MAP.get(
        tr_id, OVERSEAS_ORD_DVSN_MAP[None])
    if dvsn not in supported_dvsn:
        raise ValueError(
            f'주문 구분은 ({", ".join(i if i else "None" for i in supported_dvsn)}) 중 하나여야 합니다.')
