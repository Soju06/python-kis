...
class KisOrderableAccountProductImpl:
    ...
    def buy(
        ...
        Raises:
            ...
            ValueError: 종목 코드가 올바르지 않은 경우
        """
        if not isinstance(self, KisOrderableAccountProduct) or not isinstance(
            self, KisAccountProductProtocol
        ):
            raise TypeError(
                f"Invalid type: {type(self)} is not KisOrderableAccountProduct or KisAccountProductProtocol"
            )
